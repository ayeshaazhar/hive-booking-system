...
//   const { user } = useAuth()
//   const { getUpcomingBookings, bookings, isLoaded } = useBooking()
              ...
              <div className="space-y-4">
                {upcomingBookings.slice(0, 3).map((booking) => (
                  <div key={booking.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-4">
                      <div className="flex-shrink-0">
                        {booking.type === "Meeting Room" ? <Users className="h-8 w-8 text-blue-600" /> :
                         booking.type === "Phone Booth" ? <Phone className="h-8 w-8 text-green-600" /> :
                         <Calendar className="h-8 w-8 text-purple-600" />}
                      </div>
                      <div>
                        <h4 className="font-medium">{booking.resource}</h4>
